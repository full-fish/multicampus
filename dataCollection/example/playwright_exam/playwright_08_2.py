...
async def main(num):
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            print("page_data개수", len(page_data))

            tasks = [
                insert_data(pool, page_data) for page_data in page_data if page_data
            ]
...
async def insert_data(pool, page_data):
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            values = [(quote["author"], quote["text"]) for quote in page_data]
            await cur.executemany(
                "INSERT INTO quotes (author, text) VALUES (%s, %s)", values
            )
        await conn.commit()
    print(f"{len(page_data)}개의 명언이 데이터베이스에 저장되었습니다.")
